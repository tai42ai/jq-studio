...
export function generateValueExpression(
  ...
  switch (data.valueType) {
    case ValueType.String:
      return `"${escapeJqString((data.value ?? '') as string)}"`;
    ...
    case ValueType.Array:
      return formatArray(buildArrayItems(node, data, context, nodeExpressionFn, indent), indent);

    case ValueType.Object:
      return formatObject(buildObjectFields(node, data, context, nodeExpressionFn, indent), indent);
...
/**
 * Builds the item expressions of an Array node, in the order the node declares
 * its items rather than the order the edges were drawn.
 *
 * @throws {Error} If an item edge names an item the node does not declare
 */
function buildArrayItems(
  ...
  return itemEdges.map(({ edge }) => {
    ...
    return asTerm(itemExpr);
  });
}

/**
 * Builds the `"key": value` field expressions of an Object node, in the order
 * the node declares its fields rather than the order the edges were drawn.
 *
 * @throws {Error} If a field edge names a field the node does not declare
 */
function buildObjectFields(
  ...
  return fieldEdges.map(({ edge, fieldMeta }) => {
    ...
    return `"${escapeJqString(fieldMeta.name)}": ${asTerm(valueExpr)}`;
  });
}
